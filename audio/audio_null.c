...
{
	DEBUG(1, "audio_null: init()\n");
	return 0;
}

static void shutdown(void)
{
	DEBUG(1, "audio_null: shutdown()\n");
}
...
{
	DEBUG(2, "audio_null: free_sound(%d)\n", soundno);
}
...
{
	DEBUG(2, "audio_null: play_sound_at(%d, %.3f, %.3f, %.3f, %d, %#x)\n",
...
{
	DEBUG(2, "audio_null: stop_sound(%d)\n", handle);
	return 0;
}
